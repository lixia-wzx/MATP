...
def get_lane_centerline_index(centerline_position):
    num_centerline = [i.shape[0] for i in centerline_position]
...
class MapEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.c_emb_ctrs = TwoLayerMLP(input_dim=2, hidden_dim=hidden_dim, output_dim=hidden_dim)
        self.c_emb_feats = TwoLayerMLP(input_dim=2, hidden_dim=hidden_dim, output_dim=hidden_dim)
        self.c_emb_attr = TwoLayerMLP(input_dim=2, hidden_dim=hidden_dim, output_dim=hidden_dim)

        self.l_emb_ctrs = TwoLayerMLP(input_dim=2, hidden_dim=hidden_dim, output_dim=hidden_dim)
        self.l_emb_feats = TwoLayerMLP(input_dim=2, hidden_dim=hidden_dim, output_dim=hidden_dim)
        self.l_emb_attr = TwoLayerMLP(input_dim=2, hidden_dim=hidden_dim, output_dim=hidden_dim)
    ...
    def forward(self, data):
        ...
        c_ctrs = self.c_emb_ctrs(c2l_position_c)
        c_feats = self.c_emb_feats(c2l_feats_c)
        c_other_attr = self.c_emb_attr(torch.cat([c2l_heading_c.unsqueeze(-1), c_length.unsqueeze(-1)], dim=-1))
        c_embs = F.relu(c_ctrs + c_feats + c_other_attr)
        ...
        l_ctrs = self.l_emb_ctrs(c2l_position_l)
        l_feats = self.l_emb_feats(c2l_feats_l)
        l_other_attr = self.l_emb_attr(torch.cat(
            [l_length.unsqueeze(-1), c2l_heading_l.unsqueeze(-1), l_is_intersection, l_turn_direction,
             l_traffic_control], dim=-1))
        l_embs = F.relu(l_ctrs + l_feats + l_other_attr)
